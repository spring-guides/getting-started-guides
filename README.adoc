...
"ApiAuthenticationSuccessHandler.java" is where the access token is generated, put in a simple cache and returned to client after a sucessful authentication:
...
"ApiPreAuthUserDetailsService.java" is where the access token is checked whether it was a previously authenticated or not and if it is a valid token, then User details are loaded from cache :
