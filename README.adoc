...
Lets create a route the routes a request to `http://httpbin.org/get` when a request is
made to the gateway at `/get`.  In addition we will add a filter that will add the
...
To test this out just start the Gateway, it should be running on port `8080`.  Then make
a request to `http://localhost:8080/get`.  You can do this using cURL by issuing the following
...
Another feature in the Spring Cloud Gateway is to provide resiliency to your clients
by using circuit breakers via Hystrix.  This is implemented via a simple filter that
you can add to your requests.  Lets create another route to demonstrate this.

In this example we will leverage HTTPBin's delay API that waits a certain number of
seconds before sending a response.  This is a good feature to let you test how your
API handles slow responses.  In our case we want Hystrix to timeout and protect our
clients from downstream services that are taking too long to respond.  Add a new route
to our `RouteLocator` that looks like the following
...
As you can see Hystrix timed out waiting for the response from HTTPBin.  When Hystrix time's out
we can optionaly provide a fallback so that clients to not ust received a `504` but something
more meaninful.  In a prouction scenario you may return some data from a cache for example
...
In `Application.java` create a new `static` class called `UriConfiguration`.
...
That's a lot of functionality in just 37 lines of code!
...
setup "stub" for the HTTPBin APIs we call via the Gateway and mock the behavior we expect.
